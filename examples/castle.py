...
async def solve_captcha_sync(num_requests):
    return [await cap_monster_client.solve_captcha(castle_request) for _ in range(num_requests)]


async def solve_captcha_async(num_requests):
    tasks = [asyncio.create_task(cap_monster_client.solve_captcha(castle_request))
...
if __name__ == '__main__':
    ...
    castle_request = CastleCustomTaskRequest(
